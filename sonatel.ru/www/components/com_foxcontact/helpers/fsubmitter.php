...
class FSubmitter extends FDataPump {
	...
	public function Show() {
		...
		$result .= '<div class="form-field foxfield fxf-submit">' . PHP_EOL;

		$buttonSfxClass = str_replace(' ', '-', trim($this->Params->get('pageclass_sfx'))) . '-btn';
		switch ($this->Params->get("submittype")) {
			case 1:
				// Submit input
				$result .= '<input class="foxbutton ' . $buttonSfxClass . ' btn" type="submit" name="' .
						$this->GetId() . '" value="' . $this->Params->get("submittext") . '"/>' . PHP_EOL;
				break;
			
			case 2:
				// Submit link
				$result .= '<a class="foxbutton ' . $buttonSfxClass . ' btn" '
					. 'onclick="jQuery(\'#' . $this->GetId() . '\').click();" href="#">' . 
					$this->Params->get("submittext") . '</a>' . PHP_EOL .
					'<input style="display:none;" type="submit" id="' .
						$this->GetId() . '" name="' .
						$this->GetId() . '" value="' . $this->Params->get("submittext") . '"/>' . PHP_EOL;
				break;
			...
			default:
				...
				$result .= '<button ';
				if ($this->Params->get("submittext") != '') {
					$result .= ' class="foxbutton ' . $buttonSfxClass . ' btn" ';
				}
				$result .= ' type="submit" name="' . $this->GetId() . '">';
				if ($icon != "-1")
					$result .= PHP_EOL .
							'<span style="background: url(' . JUri::base(true) . '/media/' .
							$GLOBALS["com_name"] . '/images/submit/' . $icon . ') no-repeat scroll ' .
							$GLOBALS["left"] . ' top transparent; padding-' . $GLOBALS["left"] . ':20px;" >' . PHP_EOL;
				$result .= $this->Params->get("submittext");
				if ($icon != "-1")
					$result .= '</span>' . PHP_EOL;
			...
			switch ($this->Params->get("resettype")) {
				...
				default:
					...
					$result .= '<button class="foxbutton btn" type="reset" onClick="ResetFoxControls();">' . PHP_EOL .
							'<span ';
					if ($reseticon != "-1")
						$result .= 'style="background: url(' . JUri::base(true) . '/media/' . $GLOBALS["com_name"] . '/images/reset/' . $reseticon . ') no-repeat scroll ' . $GLOBALS["left"] . ' top transparent; padding-' . $GLOBALS["left"] . ':20px;" ';
					$result .= '>' . PHP_EOL .
	...
	private function BuildCheckboxField($key, &$field) {
		...
		$result = '<div class="fox_copy_to_sender"><input ' .
				'type="checkbox" ' .
				'class="foxcheckbox" ' .
				"value=\"" . JText::_('JYES') . "\" " .
				$checked .
