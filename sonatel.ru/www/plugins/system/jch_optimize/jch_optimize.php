...
class plgSystemJCH_Optimize extends JPlugin {
    ...
    public function onAfterRender() {
        ...
        $sBody = preg_replace($sHeadRegex, $this->sHead, $this->sBody);
				...
				// FIX move inline scripts to bottom			
				if ($iJavaScript) {
						$sBody = $this->moveInlineScripts($sBody);
				}
		...
		protected function moveInlineScripts($body) {
			$this->inlineScripts = array();
			$body = preg_replace_callback('{<script([^>]*)>(.*?)</script>}is',
					array($this, 'replaceInlineScripts'), $body);
			$body = str_replace('</body>', implode("\n", $this->inlineScripts) . '</body>', $body);
						
			return $body;
		}
		
		protected function replaceInlineScripts($m) {
			// ignore files
			if (strpos($m[1], 'src=') !== false) {
				return $m[0];
			}
			// ignore other types
			if (strpos($m[1], 'type=') !== false && strpos($m[1], 'text/javascript') === false) {
				return $m[0];
			}
			
			$this->inlineScripts[] = $m[0];			
