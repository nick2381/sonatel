 ...
 */
var MCColorChooser={add:function(h,i){var f=h.replace(/-/,"_"),j;if(!window.moorainbow){window.moorainbow={};}var g=function(){var a=document.id(h);a.getParent().removeEvent("mouseenter",g);
j=new MooRainbow("myRainbow_"+h+"_input",{id:"myRainbow_"+h,startColor:document.id(h).get("value").hexToRgb(true)||[255,255,255],imgPath:MCURL+"/fields/colorchooser/images/",transparent:i,onChange:function(b){if(b=="transparent"){a.getNext().getFirst().addClass("overlay-transparent").setStyle("background-color","transparent");
...
this.fireEvent("keyup");});a.getNext().getFirst().setStyle("background-color",j.sets.hex);MCColorChooser.load("myRainbow_"+h);};if(f.contains("gradient")&&(f.contains("from")||f.contains("to"))){g();
